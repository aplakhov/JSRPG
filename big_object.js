class BigScaryObject {
    ...
    isVisible(pixelOffset) {
        ...
        if (!img)
            return false;
        const rotated = this.rotation.get();
        if (rotated)
            return true;
        const left = this.pixelX.get() - this.zeroX;
        const top = this.pixelY.get() - this.zeroY;
        if (left + img.width < pixelOffset.x)
            return false;
        if (top + img.height < pixelOffset.y)
            return false;
        if (left >= pixelOffset.x + viewInPixels)
            return false;
        if (top >= pixelOffset.y + viewInPixels)
            return false;
        return true;
    }
